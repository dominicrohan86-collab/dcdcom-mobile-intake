...
function InquiryCard({ row, open, compact = false }) {
  ...
  return <article className={cn("relative grid grid-cols-[32px_minmax(0,1fr)] gap-2 overflow-hidden rounded-lg border border-border bg-card p-3 pl-4 text-card-foreground shadow-sm before:absolute before:inset-y-0 before:left-0 before:w-[3px]", priorityBar)}>
    <span className="grid size-9 place-items-center self-center rounded-full bg-muted text-muted-foreground"><Building2 size={20} /></span>
    <div className="min-w-0">
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0"><h3 className="text-sm font-bold leading-tight text-card-foreground">{item.company}</h3><p className="mt-1 text-xs text-foreground/80">{item.service}</p>{!compact && <p className="mt-1 flex items-center gap-1 text-xs text-muted-foreground"><MapPin size={12} /> {item.location}</p>}<p className="mt-1 flex items-center gap-1 text-xs text-muted-foreground"><UserRound size={12} /> {item.owner_name || "Unassigned"}</p></div>
        <Badge tone={stageTones[item.status] || "slate"} className="shrink-0">{stageLabels[item.status] || "New"}</Badge>
      </div>
      <div className="mt-2 flex items-center justify-between gap-2">
        {item.missingCount > 0 ? <span className="inline-flex items-center gap-1 rounded bg-amber-500/15 px-2 py-1 text-xs text-amber-700 dark:text-amber-300"><AlertTriangle size={13} />{item.missingCount} missing</span> : <span className="text-xs font-medium text-emerald-700 dark:text-emerald-300">Ready to progress</span>}
...
function LoadingRows() {
  return <>{[1, 2, 3].map((item) => <div key={item} className="h-24 animate-pulse rounded-md bg-muted" />)}</>;
}

...
import {
  activityEvents, aiRuns, aiSummaries, auditLog, chatFiles, chatMessages, chatSessions, chatSources, checklistItems, communicationDeliveryAttempts,
...
} from "../../../db/schema/drizzle-schema.js";
...
export async function listChatSessions(env, accountId, userId, filters = {}) {
  const db = getDb(env);
  const predicates = [eq(chatSessions.accountId, accountId), eq(chatSessions.createdByUserId, userId)];
  if (filters.scope) predicates.push(eq(chatSessions.scope, filters.scope));
  if (filters.inquiryId) predicates.push(eq(chatSessions.inquiryId, filters.inquiryId));
  const rows = await db.select().from(chatSessions).where(and(...predicates)).orderBy(desc(chatSessions.updatedAt)).limit(Math.min(Number(filters.limit || 20), 50));
  return rows.map(publicChatSession);
}

export async function createChatSession(env, accountId, userId, payload = {}) {
  const scope = payload.scope === "inquiry" ? "inquiry" : "workspace";
  const inquiryId = scope === "inquiry" ? payload.inquiryId : null;
  if (inquiryId && !await getInquiryDetail(env, accountId, inquiryId, userId)) return null;
  const sessionId = id("chat");
  const title = String(payload.title || (scope === "inquiry" ? "Inquiry assistant" : "Workspace assistant")).slice(0, 120);
  const value = { id: sessionId, accountId, createdByUserId: userId, scope, inquiryId, title, status: "active", createdAt: now(), updatedAt: now() };
  await getDb(env).insert(chatSessions).values(value);
  return publicChatSession(value);
}

export async function getChatSession(env, accountId, userId, sessionId) {
  const [row] = await getDb(env).select().from(chatSessions).where(and(eq(chatSessions.accountId, accountId), eq(chatSessions.createdByUserId, userId), eq(chatSessions.id, sessionId))).limit(1);
  return row ? publicChatSession(row) : null;
}

export async function ensureChatSession(env, accountId, userId, sessionId, payload = {}) {
  if (sessionId) {
    const existing = await getChatSession(env, accountId, userId, sessionId);
    if (!existing) return null;
    return existing;
  }
  return createChatSession(env, accountId, userId, payload);
}

export async function listChatMessages(env, accountId, userId, sessionId) {
  const session = await getChatSession(env, accountId, userId, sessionId);
  if (!session) return null;
  const db = getDb(env);
  const rows = await db.select().from(chatMessages).where(and(eq(chatMessages.accountId, accountId), eq(chatMessages.sessionId, sessionId))).orderBy(asc(chatMessages.createdAt));
  const messageIds = rows.map((row) => row.id);
  const sourceRows = messageIds.length ? await db.select().from(chatSources).where(inArray(chatSources.messageId, messageIds)).orderBy(asc(chatSources.label)) : [];
  const byMessage = new Map(messageIds.map((id) => [id, []]));
  for (const source of sourceRows) byMessage.get(source.messageId)?.push(publicChatSource(source));
  return { session, messages: rows.map((message) => ({ ...publicChatMessage(message), sources: byMessage.get(message.id) || [] })) };
}

export async function createChatMessage(env, accountId, session, role, body, userId, metadata = {}) {
  const db = getDb(env);
  const messageId = id("chatmsg");
  const inquiryId = session.scope === "inquiry" ? session.inquiryId : metadata.inquiryId || null;
  const value = { id: messageId, sessionId: session.id, accountId, inquiryId, role, body: String(body || "").slice(0, 40_000), metadataJson: JSON.stringify(metadata || {}), createdByUserId: userId || null, createdAt: now() };
  await db.insert(chatMessages).values(value);
  await db.update(chatSessions).set({ updatedAt: now(), title: session.title === "Workspace assistant" || session.title === "Inquiry assistant" ? chatTitleFrom(body, session.title) : session.title }).where(eq(chatSessions.id, session.id));
  return publicChatMessage(value);
}

export async function createChatSources(env, messageId, sources = []) {
  const values = sources.slice(0, 12).map((source) => ({
    id: id("chatsrc"),
    messageId,
    sourceType: String(source.type || source.sourceType || "record").slice(0, 80),
    sourceId: String(source.id || source.sourceId || "unknown").slice(0, 160),
    label: String(source.label || "Source").slice(0, 180),
    excerpt: String(source.excerpt || "").slice(0, 1000),
    confidenceScore: Number.isFinite(Number(source.confidenceScore)) ? Math.round(Number(source.confidenceScore)) : null,
    metadataJson: JSON.stringify(source.metadata || {})
  })).filter((source) => source.sourceId !== "unknown");
  if (values.length) await insertValues(getDb(env), chatSources, values);
  return values.map(publicChatSource);
}

export async function getChatMessage(env, accountId, userId, sessionId, messageId) {
  const session = await getChatSession(env, accountId, userId, sessionId);
  if (!session) return null;
  const [message] = await getDb(env).select().from(chatMessages).where(and(eq(chatMessages.accountId, accountId), eq(chatMessages.sessionId, sessionId), eq(chatMessages.id, messageId))).limit(1);
  return message ? publicChatMessage(message) : null;
}

export async function createChatFileRecord(env, accountId, session, userId, file) {
  const db = getDb(env);
  const chatFileId = id("chatfile");
  const value = {
    id: chatFileId,
    sessionId: session.id,
    accountId,
    inquiryId: session.scope === "inquiry" ? session.inquiryId : file.inquiryId || null,
    ...
    extractionStatus: file.extractionStatus || "unsupported",
    retentionExpiresAt: file.retentionExpiresAt || null,
    uploadedByUserId: userId,
    uploadedAt: now()
  };
  await db.insert(chatFiles).values(value);
  return publicChatFile(value);
}

export async function listChatFilesForSession(env, accountId, userId, sessionId) {
  const session = await getChatSession(env, accountId, userId, sessionId);
  if (!session) return null;
  const rows = await getDb(env).select().from(chatFiles).where(and(eq(chatFiles.accountId, accountId), eq(chatFiles.sessionId, sessionId))).orderBy(desc(chatFiles.uploadedAt));
  return rows.map(publicChatFile);
}

export async function buildChatContext(env, accountId, user, session, payload = {}) {
  const recent = await recentChatMessages(env, accountId, session.id);
  const chatFileRows = await getDb(env).select().from(chatFiles).where(and(eq(chatFiles.accountId, accountId), eq(chatFiles.sessionId, session.id))).orderBy(desc(chatFiles.uploadedAt)).limit(12);
  const selectedChatFiles = filterSelectedChatFiles(chatFileRows, payload.uploadedChatFileIds);
  const currentUser = { id: user.id, email: user.email, fullName: user.fullName, role: user.role };
  if (session.scope === "inquiry") {
    const detail = await getInquiryDetail(env, accountId, session.inquiryId, user.id);
    if (!detail) return null;
    const sources = [
      sourceFromInquiry(detail.inquiry),
      ...detail.fields.slice(0, 16).map(sourceFromField),
      ...detail.missing.slice(0, 16).map(sourceFromMissing),
      ...detail.summaries.slice(0, 6).map(sourceFromSummary),
      ...detail.documents.slice(0, 10).map(sourceFromDocument),
      ...detail.files.slice(0, 12).map(sourceFromFile),
      ...detail.communications.slice(0, 10).map(sourceFromCommunication),
      ...detail.comments.slice(0, 10).map(sourceFromComment),
      ...detail.activity.slice(0, 12).map(sourceFromActivity),
      ...detail.siteVisits.slice(0, 8).map(sourceFromVisit),
      ...selectedChatFiles.map((file) => sourceFromChatFile(file))
    ].filter(Boolean);
    return { scope: "inquiry", currentUser, records: { inquiry: detail, chatFiles: selectedChatFiles.map(snake) }, sources: selectChatSources(rankSources(sources, payload.message), payload.selectedSourceIds), recentMessages: recent };
  }
  const hint = searchHint(payload.message);
  let list = await listInquiries(env, accountId, { search: hint, limit: 30 });
  let inquiryRows = Array.isArray(list) ? list : list.inquiries || [];
  // Natural-language workspace questions often contain verbs such as "summarize"
  // that are not record fields. If the hint is too restrictive, retain the user's
  // access boundary and fall back to their visible workspace instead of sending an
  // empty context to the model.
  if (hint && !inquiryRows.length) {
    list = await listInquiries(env, accountId, { limit: 30 });
    inquiryRows = Array.isArray(list) ? list : list.inquiries || [];
  }
  const detailRows = await Promise.all(inquiryRows.slice(0, 6).map((inquiry) => getInquiryDetail(env, accountId, inquiry.id, user.id).catch(() => null)));
  const detailSources = detailRows.filter(Boolean).flatMap((detail) => [
    sourceFromInquiry(detail.inquiry),
    ...detail.missing.slice(0, 8).map(sourceFromMissing),
    ...detail.summaries.slice(0, 3).map(sourceFromSummary),
    ...detail.documents.slice(0, 4).map(sourceFromDocument),
    ...detail.files.slice(0, 5).map(sourceFromFile),
    ...detail.communications.slice(0, 4).map(sourceFromCommunication),
    ...detail.comments.slice(0, 3).map(sourceFromComment),
    ...detail.siteVisits.slice(0, 3).map(sourceFromVisit)
  ]);
  const sources = [...inquiryRows.map(sourceFromInquiryList), ...detailSources, ...selectedChatFiles.map(sourceFromChatFile)].filter(Boolean);
  return { scope: "workspace", currentUser, records: { inquiries: inquiryRows, inquiryDetails: detailRows.filter(Boolean), chatFiles: selectedChatFiles.map(snake) }, sources: selectChatSources(rankSources(sources, payload.message), payload.selectedSourceIds), recentMessages: recent };
}
...
export async function getAiUsageReport(env, accountId) {
  const db = getDb(env);
  const [runs, chatMessageCount, chatSessionCount, chatFileCount] = await Promise.all([
    db.select().from(aiRuns).where(eq(aiRuns.accountId, accountId)).orderBy(desc(aiRuns.createdAt)).limit(200),
    db.select({ value: count(chatMessages.id) }).from(chatMessages).where(eq(chatMessages.accountId, accountId)),
    db.select({ value: count(chatSessions.id) }).from(chatSessions).where(eq(chatSessions.accountId, accountId)),
    db.select({ value: count(chatFiles.id) }).from(chatFiles).where(eq(chatFiles.accountId, accountId))
  ]);
  const byRunType = groupCount(runs, "runType");
  const byStatus = groupCount(runs, "status");
  const byProvider = groupCount(runs, "provider");
  const latencyValues = runs.map((run) => Number(run.latencyMs || 0)).filter((value) => value > 0);
  const assistantRuns = runs.filter((run) => run.runType === "chat_assistant");
  return {
    generatedAt: now(),
    sampleSize: runs.length,
    totalRunsSampled: runs.length,
    chatSessions: Number(chatSessionCount[0]?.value || 0),
    chatMessages: Number(chatMessageCount[0]?.value || 0),
    chatFiles: Number(chatFileCount[0]?.value || 0),
    assistantRuns: assistantRuns.length,
    fallbackRuns: runs.filter((run) => run.status === "fallback").length,
    failedRuns: runs.filter((run) => run.status === "failed").length,
    averageLatencyMs: latencyValues.length ? Math.round(latencyValues.reduce((total, value) => total + value, 0) / latencyValues.length) : null,
    byRunType,
    byStatus,
    byProvider,
    recentRuns: runs.slice(0, 12).map((run) => ({ ...snake(run), output: safeJson(run.outputJson) || {} }))
  };
}
...
export async function runFileRetentionCleanup(env, accountId, userId, options = {}) {
  ...
  const chatRows = policy.legal_hold ? [] : await db.select().from(chatFiles).where(and(eq(chatFiles.accountId, accountId), lt(chatFiles.retentionExpiresAt, now()))).orderBy(asc(chatFiles.retentionExpiresAt)).limit(limit);
  const chatCandidates = chatRows.map(snake);
  const dryRun = options.dryRun !== false;
  const deleted = [];
  const deletedChatFiles = [];
  ...
  if (!dryRun && chatCandidates.length) {
    if (!env?.FILES?.delete) throw new Error("File storage deletion is not available.");
    for (const candidate of chatCandidates) {
      await env.FILES.delete(candidate.storage_key);
      await db.delete(chatFiles).where(eq(chatFiles.id, candidate.id));
      deletedChatFiles.push(candidate.id);
    }
  }
  await createAuditLog(env, accountId, userId, "file_retention_policy", accountId, dryRun ? "file_retention_policy.cleanup_previewed" : "file_retention_policy.cleanup_run", null, { cutoff, dryRun, candidateCount: candidates.length, deletedCount: deleted.length, chatCandidateCount: chatCandidates.length, chatDeletedCount: deletedChatFiles.length, legalHold: policy.legal_hold });
  return { policy, dryRun, cutoff, candidateCount: candidates.length, deletedCount: deleted.length, chatCandidateCount: chatCandidates.length, chatDeletedCount: deletedChatFiles.length, legalHold: policy.legal_hold, candidates: candidates.map(publicRetentionCandidate), chatCandidates };
}
...
function publicChatSession(row) {
  const item = snake(row);
  return {
    id: item.id,
    scope: item.scope,
...
async function recentChatMessages(env, accountId, sessionId) {
  const rows = await getDb(env).select().from(chatMessages).where(and(eq(chatMessages.accountId, accountId), eq(chatMessages.sessionId, sessionId))).orderBy(desc(chatMessages.createdAt)).limit(8);
  return rows.reverse().map((row) => ({ role: row.role, body: String(row.body || "").slice(0, 1200), createdAt: row.createdAt }));
}
function chatTitleFrom(body, fallback) {
  const text = String(body || "").replace(/\s+/g, " ").trim();
  return text ? text.slice(0, 80) : fallback;
}
function filterSelectedChatFiles(rows, selectedIds = []) {
  if (!Array.isArray(selectedIds) || !selectedIds.length) return rows;
  const selected = new Set(selectedIds.map(String));
  return rows.filter((row) => selected.has(String(row.id)) || selected.has(String(row.fileId || row.file_id || "")));
}
function selectChatSources(sources, selectedIds = []) {
  if (!Array.isArray(selectedIds) || !selectedIds.length) return sources;
  const selected = new Set(selectedIds.map(String));
  const matching = sources.filter((source) => selected.has(String(source.id)) || selected.has(`${source.type}:${source.id}`));
  const remaining = sources.filter((source) => !matching.includes(source));
  return [...matching, ...remaining].slice(0, Math.max(12, matching.length));
}
function searchHint(message) {
  const words = String(message || "").toLowerCase().match(/[a-z0-9][a-z0-9_-]{2,}/g) || [];
  const ignored = new Set(["what", "which", "show", "find", "with", "that", "this", "from", "need", "needs", "missing", "inquiry", "inquiries"]);
  return words.filter((word) => !ignored.has(word)).slice(0, 4).join(" ");
}
function rankSources(sources, message) {
  const terms = new Set((String(message || "").toLowerCase().match(/[a-z0-9][a-z0-9_-]{2,}/g) || []).slice(0, 16));
  const uniqueSources = Array.from(new Map(sources.map((source) => [`${source.type}:${source.id}`, source])).values());
  return uniqueSources.map((source, index) => {
    const haystack = `${source.label} ${source.excerpt}`.toLowerCase();
    const matches = [...terms].filter((term) => haystack.includes(term)).length;
    return { source, score: matches * 10 + (source.confidenceScore || 60) - index };
  }).sort((left, right) => right.score - left.score).map(({ source }) => source).slice(0, 28);
}
function sourceFromInquiry(inquiry) {
  if (!inquiry?.id) return null;
  const location = [inquiry.site_name, inquiry.city, inquiry.region].filter(Boolean).join(", ");
  return {
    type: "inquiry",
    id: inquiry.id,
    label: inquiry.title || "Inquiry",
    excerpt: [`Status ${stageLabel(inquiry.status)}`, inquiry.company_name, inquiry.contact_name, location, inquiry.access_notes ? `Access: ${inquiry.access_notes}` : null].filter(Boolean).join(" | "),
    confidenceScore: inquiry.confidence_score || 80,
    metadata: { route: `/inquiries/${encodeURIComponent(inquiry.id)}`, status: inquiry.status, priority: inquiry.priority }
  };
}
function sourceFromInquiryList(inquiry) {
  if (!inquiry?.id) return null;
  return {
    type: "inquiry",
    id: inquiry.id,
    label: inquiry.title || "Inquiry",
    excerpt: [`${inquiry.company_name || "Customer"} - ${stageLabel(inquiry.status)}`, inquiry.service_type, Number(inquiry.missing_count || 0) ? `${Number(inquiry.missing_count)} missing requirements` : null, [inquiry.city, inquiry.region].filter(Boolean).join(", ")].filter(Boolean).join(" | "),
    confidenceScore: inquiry.confidence_score || 72,
    metadata: { route: `/inquiries/${encodeURIComponent(inquiry.id)}`, status: inquiry.status, priority: inquiry.priority }
  };
}
function sourceFromField(field) {
  return field?.id ? { type: "extracted_field", id: field.id, label: field.label || field.field_key, excerpt: String(field.value_text || field.value_json || "").slice(0, 900), confidenceScore: field.confidence_score || 70, metadata: { verified: Boolean(field.is_verified) } } : null;
}
function sourceFromMissing(item) {
  return item?.id ? { type: "missing_requirement", id: item.id, label: item.label, excerpt: `${stageLabel(item.status)} ${item.category || ""} ${item.severity || ""}. ${item.notes || ""}`.trim(), confidenceScore: 80, metadata: { status: item.status, severity: item.severity } } : null;
}
function sourceFromSummary(item) {
  return item?.id ? { type: "ai_summary", id: item.id, label: `${stageLabel(item.summary_type)} summary`, excerpt: item.body || "", confidenceScore: item.confidence_score || 70, metadata: { modelName: item.model_name, sourceStatus: "draft" } } : null;
}
function sourceFromDocument(item) {
  return item?.id ? { type: "document", id: item.id, label: item.title || "Document", excerpt: String(item.body || item.subject || "").slice(0, 1000), confidenceScore: item.status === "approved" ? 90 : 70, metadata: { status: item.status, documentType: item.document_type, sourceStatus: item.status === "approved" ? "approved" : "draft", route: `/inquiries/${encodeURIComponent(item.inquiry_id)}/documents` } } : null;
}
function sourceFromFile(file) {
  return file?.id ? { type: "file", id: file.id, label: file.file_name || "File", excerpt: `${file.category || "file"} ${file.content_type || ""} ${file.size_bytes ? `${Math.round(file.size_bytes / 1024)} KB` : ""}`.trim(), confidenceScore: 65, metadata: { category: file.category, url: `/api/files/${encodeURIComponent(file.id)}` } } : null;
}
function sourceFromCommunication(item) {
  return item?.id ? { type: "communication", id: item.id, label: item.subject || `${stageLabel(item.direction)} ${item.channel}`, excerpt: String(item.body || "").slice(0, 1000), confidenceScore: 78, metadata: { channel: item.channel, direction: item.direction, status: item.status } } : null;
}
function sourceFromComment(item) {
  return item?.id ? { type: "comment", id: item.id, label: item.author_name ? `Comment by ${item.author_name}` : "Internal comment", excerpt: String(item.body || "").slice(0, 1000), confidenceScore: 72, metadata: { authorName: item.author_name || null } } : null;
}
function sourceFromActivity(item) {
  return item?.id ? { type: "activity_event", id: item.id, label: stageLabel(item.event_type), excerpt: item.summary || "", confidenceScore: 66, metadata: { eventType: item.event_type } } : null;
}
function sourceFromVisit(item) {
  return item?.id ? { type: "site_visit", id: item.id, label: "Site visit", excerpt: [`Status ${stageLabel(item.status)}`, item.scheduled_start, item.notes, ...(item.checklistItems || []).slice(0, 4).map((check) => `${check.label}: ${check.status}`)].filter(Boolean).join(" | "), confidenceScore: 76, metadata: { status: item.status } } : null;
}
function sourceFromChatFile(file) {
  if (!file?.id) return null;
  return {
    type: "chat_file",
    id: file.id,
    label: file.fileName || file.file_name || "Chat upload",
    excerpt: String(file.extractedText || file.extracted_text || `${file.contentType || file.content_type || "file"} uploaded for this chat`).slice(0, 1000),
    confidenceScore: file.extractionStatus === "complete" || file.extraction_status === "complete" ? 78 : 55,
    metadata: { extractionStatus: file.extractionStatus || file.extraction_status, fileId: file.fileId || file.file_id || null }
  };
}

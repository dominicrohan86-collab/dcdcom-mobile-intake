...
export const chatScope = z.enum(["workspace", "inquiry"]);
export const chatResponseMode = z.enum(["answer", "draft", "extract", "compare"]);
...
export const savedViewSchema = z.object({
  screen: z.enum(["today", "inquiries", "docs", "composers", "assistant", "admin"]),
...
export const settingsSchema = z.object({
  ...
  defaultView: z.enum(["today", "pipeline", "assistant", "docs", "more"]).default("today"),
...
export const chatSessionQuerySchema = z.object({
  scope: chatScope.optional(),
  inquiryId: z.string().trim().max(120).optional(),
  limit: z.coerce.number().int().min(1).max(50).default(20)
});
export const chatSessionSchema = z.object({
  scope: chatScope.default("workspace"),
  inquiryId: z.string().trim().max(120).nullable().optional(),
  title: z.string().trim().min(1).max(120).optional()
}).refine((value) => value.scope !== "inquiry" || Boolean(value.inquiryId), "Inquiry chat requires an inquiryId.");
export const chatMessageSchema = z.object({
  message: z.string().trim().min(1).max(8000),
  scope: chatScope.optional(),
  inquiryId: z.string().trim().max(120).nullable().optional(),
  selectedSourceIds: z.array(z.string().trim().max(160)).max(32).default([]),
  uploadedChatFileIds: z.array(z.string().trim().max(160)).max(16).default([]),
  responseMode: chatResponseMode.default("answer")
});
export const chatSaveNoteSchema = z.object({
  inquiryId: z.string().trim().max(120).optional(),
  messageId: z.string().trim().max(120).optional(),
  body: z.string().trim().max(8000).optional()
}).refine((value) => Boolean(value.messageId || value.body), "Provide a messageId or body to save.");
export const chatCreateDraftSchema = z.object({
  inquiryId: z.string().trim().max(120).optional(),
  messageId: z.string().trim().max(120).optional(),
  documentType: documentType.default("other"),
  title: z.string().trim().max(180).optional(),
  subject: z.string().trim().max(220).nullable().optional(),
  body: z.string().trim().max(40_000).optional()
}).refine((value) => Boolean(value.messageId || value.body), "Provide a messageId or body to create a draft.");

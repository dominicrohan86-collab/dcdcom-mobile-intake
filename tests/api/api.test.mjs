...
import { app } from "../../src/server/routes/app.js";
import { handleApi } from "../../src/server/api.js";
import { describeGoogleCalendarFailure } from "../../src/server/integrations/google-calendar.js";
import { createLocalEnv } from "../../scripts/dev/local-runtime.mjs";
...
try {
  ...
  assert(aiPrompts.body.prompts.some((prompt) => prompt.id === "chat_assistant.v2026-07-22"), "admin AI prompt registry should expose active chat assistant prompt versions");
  const aiUsage = await request(env, "GET", "/api/admin/ai-usage");
  assert(aiUsage.status === 200 && typeof aiUsage.body.usage.totalRunsSampled === "number" && typeof aiUsage.body.usage.byRunType === "object", "admin AI usage should expose aggregated AI run telemetry");

  const chatConfig = await request(env, "GET", "/api/chat/config");
  assert(chatConfig.status === 200 && chatConfig.body.enabled === true && chatConfig.body.workspaceEnabled === true, "chat config should expose enabled assistant capabilities");
  const workspaceChat = await request(env, "POST", "/api/chat/sessions", { scope: "workspace", title: "Workspace assistant test" });
  assert(workspaceChat.status === 201 && workspaceChat.body.session.scope === "workspace", "workspace assistant sessions should be creatable");
  const workspaceChatAnswer = await request(env, "POST", `/api/chat/sessions/${workspaceChat.body.session.id}/messages`, {
    message: "Summarize the current open inquiries.",
    scope: "workspace",
    responseMode: "answer"
  });
  assert(workspaceChatAnswer.status === 201 && workspaceChatAnswer.body.sources.length > 0 && workspaceChatAnswer.body.answer.includes("visible inquiries"), "generic workspace questions should fall back to visible records when search hints do not match record fields");
  const inquiryChat = await request(env, "POST", "/api/chat/sessions", { scope: "inquiry", inquiryId: seedInquiryId, title: "Inquiry assistant test" });
  assert(inquiryChat.status === 201 && inquiryChat.body.session.inquiryId === seedInquiryId, "inquiry assistant sessions should be tied to an inquiry");
  const chatAnswer = await request(env, "POST", `/api/chat/sessions/${inquiryChat.body.session.id}/messages`, {
    message: "What information is missing before estimating this project?",
    scope: "inquiry",
    inquiryId: seedInquiryId,
    responseMode: "answer"
  });
  assert(chatAnswer.status === 201 && chatAnswer.body.answer && chatAnswer.body.metadata.promptVersionId === "chat_assistant.v2026-07-22", "chat message endpoint should return a persisted assistant answer with prompt lineage");
  assert(chatAnswer.body.metadata.fallback === true && chatAnswer.body.sources.length > 0, "chat assistant fallback should cite available application context");
  const chatMessages = await request(env, "GET", `/api/chat/sessions/${inquiryChat.body.session.id}/messages`);
  assert(chatMessages.status === 200 && chatMessages.body.messages.length >= 2 && chatMessages.body.messages.some((message) => message.sources?.length), "chat message listing should include persisted conversation and source citations");
  const savedAssistantNote = await request(env, "POST", `/api/chat/sessions/${inquiryChat.body.session.id}/actions/save-note`, { body: "Assistant summary worth keeping." });
  assert(savedAssistantNote.status === 201 && savedAssistantNote.body.comment.body.includes("Assistant note"), "assistant answers should be saveable as internal notes");
  const assistantDraft = await request(env, "POST", `/api/chat/sessions/${inquiryChat.body.session.id}/actions/create-draft`, { body: "Draft created from assistant context.", documentType: "other", title: "Assistant Draft" });
  assert(assistantDraft.status === 201 && assistantDraft.body.document.metadata.source === "chat_assistant", "assistant answers should be saveable as generated drafts");
  const tenantChatRead = await request(secureEnv, "GET", `/api/chat/sessions/${inquiryChat.body.session.id}/messages`, undefined, { headers: signedTenantHeaders });
  assert(tenantChatRead.status === 404, "chat sessions must remain tenant-isolated");
  const disabledWorkspaceChat = await request({ ...env, ASSISTANT_WORKSPACE_ENABLED: "false" }, "POST", "/api/chat/sessions", { scope: "workspace", title: "Blocked workspace chat" });
  assert(disabledWorkspaceChat.status === 403 && disabledWorkspaceChat.body.code === "workspace_assistant_disabled", "workspace assistant feature flag should block workspace chat");

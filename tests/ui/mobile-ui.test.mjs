...
const files = {
  ...
  assistant: await readFile("src/client/screens/Assistant.jsx", "utf8"),
...
for (const nav of ["Today", "Inquiries", "Add", "Assistant", "More"]) {
...
assert(files.shell.includes("lg:hidden") && files.shell.includes("h-[76px]") && files.shell.includes("grid-cols-5") && files.shell.includes("const mobileNavItems"), "Bottom navigation should keep five stable mobile touch targets with a centered add action");
...
assert(files.composers.includes("navigate?.(\"assistant\", { inquiry: true })") && files.composers.includes("<Bot size={15} />Ask"), "Email and proposal composers should expose inquiry-scoped assistant entry");

for (const token of ["chatConfig", "chatSessions", "createChatSession", "chatMessages", "sendChatMessage", "chatFiles", "uploadChatFile", "saveChatNote", "createChatDraft"]) {
  assert(files.api.includes(token), `Client API should expose assistant method ${token}`);
}
for (const token of ["client.chatConfig", "client.chatSessions", "client.chatMessages", "client.chatFiles", "client.uploadChatFile", "AbortController", "Stop", "Save note", "Create draft", "SourceChip", "responseMode", "workspacePrompts", "inquiryPrompts"]) {
  assert(files.assistant.includes(token), `Assistant screen should include ${token}`);
}
assert(files.assistant.includes("scope === \"inquiry\"") && files.assistant.includes("scope === \"workspace\""), "Assistant should support both inquiry and workspace scopes");
assert(files.assistant.includes("accept=\"image/*,.pdf,.txt,.csv,.doc,.docx,.xlsx\""), "Assistant upload control should accept photos and relevant business documents");
assert(files.docs.includes("chatCandidates") && files.docs.includes("chatCandidateCount"), "File retention preview should include chat-only upload retention");

for (const endpoint of ["today", "inquiries/from-source", "inquiries/${id}/files", "files/${id}", "inquiries/${id}/send-follow-up", "inquiries/${id}/proposal-review", "chat/config", "chat/sessions", "chat/sessions/${sessionId}/messages", "chat/sessions/${sessionId}/files"]) {

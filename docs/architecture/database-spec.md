...
- Configured in `infra/cloudflare/wrangler.jsonc` as the `DB` D1 binding
- Configured in `infra/cloudflare/wrangler.jsonc` as the `FILES` R2 binding
- Canonical schema source: `db/schema/drizzle-schema.js`
...
1. Change `db/schema/drizzle-schema.js`.
...
`npm run dev` uses `scripts/dev/local-runtime.mjs` to provide Worker-compatible bindings during local development:

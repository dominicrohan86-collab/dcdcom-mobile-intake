...
Configure `AUTH_SESSION_SECRET` as a Cloudflare Worker secret before deployment. It must be a unique cryptographically random value of at least 32 characters; do not place it in `wrangler.jsonc`, `.env.example`, source code, or client-side configuration. For example:
